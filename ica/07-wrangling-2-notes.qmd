...
```{r ex_create_string}
# Your code
stringA <- 'It\'s Thursday'
stringB <- "It's Thursday"
```
...
df %>%
  mutate(wordLen = str_length(word),
         sub_word = str_sub(word, start =ceiling( wordLen / 2), end = ceiling(wordLen/2)))

dfEven <- tibble(
  word = c("zonk", "wise", "caseword")
)
```
...
```{r}
str_view(posts$title, "^(Need)")
str_count(posts$question_only, "(W|w)hat")
str_detect(posts$title, "(H|h)usband|(W|w)ife")
str_subset(posts$question_only, "in-law|in law|inlaw")
str_extract(posts$question_only, "baby")
str_extract_all(posts$title, "mother|mom")

```

```{r}
str_replace(posts$question_only, "abby", "Abby")

```

```{r}
str_replace_all(posts$question_only, "new york city", "NYC")
```

```{r}
str_remove(posts$question_only, "hate")
```
```{r}
str_remove_all(posts$title, "depressed")
```
