...
```{r}    
# for loop
ch_vector <- vector("character", 12)
for (i in month.name){
  myStr <- str_c('Month', i, month.name[i])
  ch_vector[i] <- myStr
}
...
# for loop
modelList <- list()
for (i in seq_len(unique(diamonds$cut))){
  subModel <- diamonds %>%
    filter(cut == unique(diamonds$cut)[i])
   modelList[i] <- lm(price ~ carat, data = subModel)
}
...
```{r}
transform_to_Str <- function(x){
  newStr <- str_c(x, " mm")
  return(newStr)
}
diamonds |>
  mutate(across(.cols = c(x, y, z), .fns = transform_to_Str))
```
...
```{r}
df_arg_combos <- crossing(c(carat ~ price, carat ~ price + cut, carat ~ price + cut + clarity, carat ~ price + cut + clarity + color), data = diamonds )
```
...
remove_outliers <- function(data, what_var, sd_thresh) {
  mySd<- sd(what_var)
  myMean <- mean(what_var)
  data <- data %>%
    filter(abs({{ what_var }} - myMean) <= sd_thresh*mySd)
 return(data) 
}

remove_outliers(diamonds, what_var = diamonds$carat, sd_thres = 3)
```
...
fit_model <- function(data, mod_formula, remove_outliers) {
    ...
    if (remove_outliers){
      data <- remove_outliers(data, data$carat, 3)
    }
    
    return(lm(mod_formula, data = data))
...
```{r}
for (i in length(df_arg_combos)){
  fit_model(diamonds, df_arg_combos[[i]], TRUE)
  fit_model(diamonds, df_arg_combos[[i]], FALSE)
}
