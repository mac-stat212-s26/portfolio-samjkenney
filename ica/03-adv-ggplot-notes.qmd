...
2015's temperature are contrasted with standard temperatures and historical highs and lows. 
...
ggplot(weather, aes(x = dateInYear, y = Low)) +
    geom_linerange(aes(ymin = RecordLow, ymax = RecordHigh), colour = '#ECEBE3', linewidth = 1) +
    theme_classic()
```
