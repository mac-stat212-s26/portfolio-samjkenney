...
formatPhone <- function(num){
  first3 <- '[0-9][0-9][0-9]'
  remainNum <- str_replace(num, first3, '')
  last4 <- str_replace(remainNum, first3, '')
  phoneFormat <- str_c('(', str_match(num, first3), ') ', str_match(remainNum, first3), ' ', str_match(last4, '[0-9][0-9][0-9][0-9]'))
  return(phoneFormat)
}

formatPhone(c("651-330-8661"))
...
extractDomain <- function(email) {
  domainRegex <- '@.*'
  domain <- str_match(email, domainRegex)
  justDomain <- str_sub(domain,start=2)
  if (str_detect(justDomain, "\\..")){
    return(justDomain)
  }
  else{
...
extractDomain('skenney@macalester.edu')
extractDomain('skenney@macalester')
```
