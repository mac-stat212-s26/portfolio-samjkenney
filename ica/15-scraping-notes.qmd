...
my_tibble <- tibble()
for (i in 0:4){
  this_html <- str_c("https://www.nih.gov/news-events/news-releases?page=", i)
  this_webpage <- read_html(this_html)
  headings <- this_webpage |> html_elements(".thumbnail-teaser__heading") |> html_text()
  descriptions <- this_webpage |> html_elements(".thumbnail-teaser__description") |> html_text()
  new_tibble <- tibble(title = headings, description = descriptions)
  my_tibble <- rbind(my_tibble, new_tibble)
}

clean_tibble <- my_tibble |>
  mutate(date = str_extract(string = description, pattern = ".*—"),
         abstract = str_extract(description, pattern = "—.*"),
