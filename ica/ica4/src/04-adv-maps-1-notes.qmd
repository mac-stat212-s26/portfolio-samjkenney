...
mn_water <- st_read("../data/LakesAndRivers.shp")
```
...
Not very accurate, as it's not anchored on the map
...
mn_counties_transformed <- st_transform(mn_counties, crs = st_crs(mn_cities))
...
```{r}
ggplot(mn_counties_transformed) +
  geom_sf() +
...
seven_countyarea <- mn_counties |>
    ...
    st_bbox()
seven_countyarea
county_crop <- mn_counties %>% st_crop(seven_countyarea) %>% st_cast('MULTIPOLYGON')
...
png("../figures/tc_map_zoom.png", width = 1040, height = 1040, units = "px")
