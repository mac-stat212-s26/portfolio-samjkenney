...
```{r}
getEven <- function(x){
  evens <- seq(from = 2, to = length(x), by = 2)
  return(x[evens])
}
...
```{r}
allTillLast <- function(x){
  excludeLast <- seq(from = 1, to = length(x) - 1, by = 1)
  return(x[excludeLast])
}
...
fit_mod_and_extract <- function(data, yvar, preds, pred_of_interest) {
   ...
   mod_formula_str <- str_c(yvar, '~', str_c(preds, collapse = "+"))
    ...
    tibble(
        ...
        estimate = coef(mod)[pred_of_interest],
        ci_lower = ci[1],
        ci_upper = ci[2]
    )
}
